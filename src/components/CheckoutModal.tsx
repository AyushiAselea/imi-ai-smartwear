  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      if (err.code === "auth/email-already-in-use") {
        ...
        // Email/password sign-in is disabled in Firebase Console — offer Google instead
        toast.info("Email sign-up is not enabled. Please continue with Google.");
        // Auto-trigger Google sign-in so user doesn't have to click again
        handleGoogleSignup();
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (err.code === "auth/wrong-password" || err.code === "auth/invalid-credential" || err.code === "auth/invalid-login-credentials") {
        // Could be a Google-only account — offer Google sign-in as fallback
        toast.error("Incorrect password. If you signed up with Google, use the Google button instead.");
      } else if (err.code === "auth/user-not-found") {
        toast.error("No account found. Create one first.");
        setSignupMode("form");
      } else if (err.code === "auth/operation-not-allowed") {
        toast.info("Email login is not enabled. Please continue with Google.");
        handleGoogleSignup();
      } else {
        ...
        {step === "signup_nudge" && (
          <div className="space-y-5">
            ...
            {signupMode === "login_form" && (
              <div className="space-y-3">
                {/* Google option — shown first for accounts that were created via Google */}
                <button
                  onClick={handleGoogleSignup}
                  disabled={signupLoading}
                  className="w-full flex items-center justify-center gap-2.5 py-2.5 rounded-full border border-border bg-card hover:bg-secondary transition-colors text-sm font-semibold disabled:opacity-50"
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <div className="h-px flex-1 bg-border" /><span>or use password</span><div className="h-px flex-1 bg-border" />
                </div>
                <input type="email" value={guestEmail} disabled className={`${INPUT} opacity-60 cursor-not-allowed`} />
                <div className="relative">
                  <input
